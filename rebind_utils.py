...
ALLOWABLE_FEATURES_VOCAB = {
    ...
    "possible_bond_type": ["SINGLE", "DOUBLE", "TRIPLE", "AROMATIC", "DATIVE", "misc"],
}
...
def make_mask_for_pyd_batch_graph(batch: torch.Tensor):
    """Mask the distance matrix of a PyG batch of disconnected graphs.

    Args:
        batch: Shape (n,), e.g. [0, 0, 0, 1, 1, 1, 1]. Each value is the
            graph index of that node.

    Returns:
        torch.Tensor: Shape (n, n), where n is the number of nodes in the batch.
    """
    n = batch.shape[0]
...
def valid_length_to_mask(valid_length: torch.TensorType, max_len: int | None = None):
...
def mask_attention_score(
    attention_score: torch.Tensor,
    attention_mask: torch.Tensor,
    mask_pos_value: float = 0.0,
):
    """Mask the attention score. b: batch, seq: length, h: heads.

    Args:
        attention_score (torch.Tensor): Shape (b, h, seq, seq) or (b, seq, seq).
        attention_mask (torch.Tensor): Shape (b, seq) or (b, seq, seq).
        mask_pos_value (float, optional): Value marking positions to mask.
            Defaults to 0.0.
    Returns:
        torch.Tensor: Masked score, same rank as ``attention_score``.
    """
    shape = attention_score.shape
    b, h, seq, _ = shape if len(shape) == 4 else (shape[0], 1, shape[1], shape[2])
    # (b, seq, seq) -> (b, 1, seq, seq); 4D scores stay (b, h, seq, seq)
    attention_score = attention_score.view(b, h, seq, seq) if len(shape) == 3 else attention_score
    # (b, seq) -> (b*h, seq)
    attention_mask = attention_mask.repeat_interleave(h, dim=0)  # (b*h, seq) or (b*h, seq, seq)
    attention_mask = attention_mask.view(b, h, -1, seq)  # (b, h, 1, seq) or (b, h, seq, seq)
...
def mask_hidden_state(hidden_X: torch.Tensor, padding_mask: torch.Tensor | None = None):
    """Mask the hidden state. b: batch, seq: length, d: hidden size.

    Args:
        hidden_X (torch.Tensor): Hidden state with shape (b, seq, d).
        padding_mask (torch.Tensor, optional): Per-sequence mask, shape (b, seq).
            Defaults to None.
...
def align_conformer_hat_to_conformer(
    conformer_hat: torch.Tensor,
    conformer: torch.Tensor,
    padding_mask: torch.Tensor,
    eps: float = 1e-8,
) -> torch.Tensor:
    """
    Kabsch-align conformer_hat to conformer, using padding_mask to ignore padded atoms.
    Critically: SVD is done under no_grad to avoid SvdBackward NaNs.
    """
    # padding_mask: (B,N) with 1 for real atoms, 0 for padding
    w = padding_mask.to(conformer_hat.dtype).unsqueeze(-1)  # (B,N,1)
    denom = w.sum(dim=1, keepdim=True).clamp_min(1.0)

    # masked means
    p_mean = (conformer_hat * w).sum(dim=1, keepdim=True) / denom
    q_mean = (conformer * w).sum(dim=1, keepdim=True) / denom

    # masked centered coords
    P = (conformer_hat - p_mean) * w
    Q = (conformer - q_mean) * w

    # covariance
    H = Q.transpose(1, 2) @ P  # (B,3,3)

    # compute rotation with detached SVD (prevents LinalgSvdBackward0 NaNs)
    with torch.no_grad():
        U, _S, Vh = torch.linalg.svd(H.double(), full_matrices=False)  # Vh is (B,3,3)
    ...
    R = R.to(conformer_hat.dtype)  # treat as constant in backward

    # apply rotation (grad flows to conformer_hat, but NOT through SVD)
    P_rot = (conformer_hat - p_mean) @ R
    return P_rot + q_mean
...
def get_mask_with_ratio(node_mask: torch.Tensor, ratio: float) -> torch.Tensor:
    ...
    Args:
        node_mask (torch.Tensor): 1 is a real atom, 0 is padding.
        ratio (float): Fraction of real atoms to mask.

    Returns:
        torch.Tensor: 1 is a masked atom, 0 is a kept atom.
    """
...
def get_masked_atom_mask(mask_with_ratio: torch.Tensor, node_mask: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        mask_with_ratio (torch.Tensor): 1 is a masked atom, 0 is kept.
        node_mask (torch.Tensor): 1 is a real atom, 0 is padding.
...
def compute_distance_residual_bias(
    cdist: torch.Tensor, cdist_mask: torch.Tensor, raw_max: bool = True
) -> torch.Tensor:
    b, seq, _ = cdist.shape
    D = cdist * cdist_mask
    if not raw_max:
        ...
        D = D_max.view(b, seq, 1) - D  # raw-max value subtract every raw
    D.diagonal(dim1=-2, dim2=-1)[:] = 0  # set diagonal to 0
    D = D * cdist_mask
    return D


def get_sigma_and_epsilon(mol_data):
    lj_parameters = {
        ...
        36: {"sigma": 4.114, "epsilon": 0.0400},  # Rb
        37: {"sigma": 3.641, "epsilon": 0.2350},  # Sr
        38: {"sigma": 3.345, "epsilon": 0.0720},  # Y
        39: {"sigma": 3.124, "epsilon": 0.0690},  # Zr
        40: {"sigma": 3.167, "epsilon": 0.0590},  # Nb
        41: {"sigma": 3.167, "epsilon": 0.0560},  # Mo
        42: {"sigma": 3.140, "epsilon": 0.0480},  # Tc
        43: {"sigma": 3.076, "epsilon": 0.0560},  # Ru
        44: {"sigma": 3.053, "epsilon": 0.0530},  # Rh
        45: {"sigma": 3.003, "epsilon": 0.0410},  # Pd
        46: {"sigma": 3.148, "epsilon": 0.0360},  # Ag
        47: {"sigma": 2.848, "epsilon": 0.2280},  # Cd
        48: {"sigma": 4.432, "epsilon": 0.4080},  # In
        49: {"sigma": 4.295, "epsilon": 0.3810},  # Sn
        50: {"sigma": 4.314, "epsilon": 0.3110},  # Sb
        51: {"sigma": 4.382, "epsilon": 0.2920},  # Te
        52: {"sigma": 4.436, "epsilon": 0.2520},  # I
        53: {"sigma": 4.465, "epsilon": 0.2210},  # Xe
        54: {"sigma": 4.302, "epsilon": 0.0320},  # Cs
        55: {"sigma": 3.666, "epsilon": 0.2200},  # Ba
        56: {"sigma": 3.522, "epsilon": 0.0130},  # La
        57: {"sigma": 3.493, "epsilon": 0.0130},  # Ce
        58: {"sigma": 3.468, "epsilon": 0.0130},  # Pr
        59: {"sigma": 3.441, "epsilon": 0.0130},  # Nd
        60: {"sigma": 3.415, "epsilon": 0.0130},  # Pm
        61: {"sigma": 3.391, "epsilon": 0.0130},  # Sm
        62: {"sigma": 3.364, "epsilon": 0.0130},  # Eu
        63: {"sigma": 3.340, "epsilon": 0.0130},  # Gd
        64: {"sigma": 3.315, "epsilon": 0.0130},  # Tb
        65: {"sigma": 3.289, "epsilon": 0.0130},  # Dy
        66: {"sigma": 3.265, "epsilon": 0.0130},  # Ho
        67: {"sigma": 3.241, "epsilon": 0.0130},  # Er
        68: {"sigma": 3.216, "epsilon": 0.0130},  # Tm
        69: {"sigma": 3.191, "epsilon": 0.0130},  # Yb
        70: {"sigma": 3.167, "epsilon": 0.0130},  # Lu
        71: {"sigma": 3.141, "epsilon": 0.0670},  # Hf
        72: {"sigma": 3.170, "epsilon": 0.0600},  # Ta
        73: {"sigma": 3.168, "epsilon": 0.0540},  # W
        74: {"sigma": 3.111, "epsilon": 0.0460},  # Re
        75: {"sigma": 3.120, "epsilon": 0.0310},  # Os
        76: {"sigma": 3.020, "epsilon": 0.0320},  # Ir
        77: {"sigma": 2.754, "epsilon": 0.0800},  # Pt
        78: {"sigma": 3.293, "epsilon": 0.0390},  # Au
        79: {"sigma": 2.730, "epsilon": 0.4090},  # Hg
        80: {"sigma": 4.347, "epsilon": 0.3790},  # Tl
        81: {"sigma": 4.355, "epsilon": 0.3450},  # Pb
        82: {"sigma": 4.370, "epsilon": 0.2840},  # Bi
        83: {"sigma": 4.450, "epsilon": 0.2220},  # Po
        84: {"sigma": 4.750, "epsilon": 0.2840},  # At
        85: {"sigma": 4.765, "epsilon": 0.2210},  # Rn
        86: {"sigma": 3.846, "epsilon": 0.0300},  # Fr
        87: {"sigma": 3.659, "epsilon": 0.2110},  # Ra
        88: {"sigma": 3.478, "epsilon": 0.0330},  # Ac
        89: {"sigma": 3.395, "epsilon": 0.0310},  # Th
        90: {"sigma": 3.424, "epsilon": 0.0100},  # Pa
        91: {"sigma": 3.397, "epsilon": 0.0110},  # U
        92: {"sigma": 3.424, "epsilon": 0.0050},  # Np
        93: {"sigma": 3.424, "epsilon": 0.0050},  # Pu
        94: {"sigma": 3.381, "epsilon": 0.0140},  # Am
        95: {"sigma": 3.326, "epsilon": 0.0140},  # Cm
        96: {"sigma": 3.339, "epsilon": 0.0140},  # Bk
        97: {"sigma": 3.313, "epsilon": 0.0140},  # Cf
        98: {"sigma": 3.286, "epsilon": 0.0140},  # Es
        99: {"sigma": 3.281, "epsilon": 0.0140},  # Fm
        100: {"sigma": 3.268, "epsilon": 0.0140},  # Md
        101: {"sigma": 3.254, "epsilon": 0.0140},  # No
        102: {"sigma": 3.240, "epsilon": 0.0070},  # Lr
    }
    eps_list = []
    sig_list = []
